...
event init(t=0){
	if (!restore (file = "restart")) {
		...
		refine (y >= -5*hl*L0 && y <= 5*hl*L0 && level < L_maxv);
		restore (file = "init");
		scalar m1[];
		fraction(m1, hl/2. - y);
		scalar m2[];
...
//  double uemax = 0.3*normf(u.x).avg;     //0.2

/*  foreach()
  omegaz[] = (u.y[1] - u.y[-1] - u.x[0,1] + u.x[0,-1])/(2.*Delta);
  double vortmax = 300.0*normf(omegaz).avg;    //200
 adapt_wavelet2((scalar *){f1,u.x,u.y,u.z,omegaz},(double []){0.001,uemax,uemax,uemax,vortmax},(int []){L_maxf,L_maxv,L_maxv,L_maxv,L_maxv},L_min);
*/
  vorticity (u, omegaz);
  double vortmax = 300.0*normf(omegaz).avg;
 adapt_wavelet2((scalar *){f1,u.x,u.y,u.z,omegaz},(double []){0.001,0.1,0.1,0.1,vortmax},(int []){L_maxf,L_maxv,L_maxv,L_maxv,L_maxv},L_min);
}
